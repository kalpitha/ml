...
def main():
    ...
    print('Average accuracy:', float(aveAccuracy) / iterations, '%')
def train(trainData, dataDir):
    ...
    print("Words frequency : ",wordsFrequency)
...
def test(testData, dataDir, priors, wordsLikelihood):
	...
	for realClass, fileList in list(testData.items()):
		...
		print('num ' + realClass + ' correct docs:', classCorrectCount)	
...
def getTrainTestData(dataDir):
	...
	for c in classes: # directory represents each class for documents		
		...
		print('num ' + c + ' training docs : ', len(trainData[c]))
		print('num ' + c + ' test docs : ', len(testData[c]))
